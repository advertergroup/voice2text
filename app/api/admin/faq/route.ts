...
export async function POST(req: Request) {
  ...
  for (const [id, d] of Object.entries(porId)) {
    if (id === "new") {
      if (d.pregunta?.trim()) await prisma.faqItem.create({ data: { pregunta: d.pregunta.trim(), respuesta: (d.respuesta || "").trim(), orden: 99 } });
      continue;
    }
  ...
  return NextResponse.redirect(new URL("/admin/faq?saved=1", base), { status: 303 });
}
